...
def augment_data(X_train, y_train):
    ...
    X_train_flip = np.array([np.flipud(img) for img in X_train[:num_labeled, 0]])
    ...
    X_train_noise = np.array([add_noise(img) for img in X_train[:num_labeled, 0]])
    ...
    print("Originalni skup:", X_train[:num_labeled].shape)
